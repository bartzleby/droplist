...
int main ()
{
  ...
  
  {
    ...
    std::set<std::string>         stl_set;  // The built-in set class - no constructor parameters.
    cs3505::string_set          our_set;  // Our set class, with a stringset of default width 10.


    std::vector<std::string> elements;

    cs3505::string_set test_set;
    cs3505::string_set test_copy;
    ...
    elements = test_copy.get_elements();
    ...
    //    for (std::set<std::string>::iterator it = stl_set.begin(); it != stl_set.end(); it++)
    //    {
    //      std::string word = *it;
    ...
    std::cout << "STL set contains " << stl_set.size() << " unique words.\n";
    std::cout << "Our set contains " << our_set.get_size() << " unique words.\n"; 
